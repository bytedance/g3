...
mod host;
pub(crate) use host::S2nTlsHostConfig;

const SERVER_CONFIG_TYPE: &str = "S2nTlsProxy";

#[derive(Clone, Debug, PartialEq)]
pub(crate) struct S2nTlsProxyServerConfig {
    name: MetricsName,
    ...
    pub(crate) hosts: HostMatch<Arc<S2nTlsHostConfig>>,
...
impl S2nTlsProxyServerConfig {
    ...
    pub(super) fn parse(
        ...
        let mut server = S2nTlsProxyServerConfig::new(position);
...
impl ServerConfig for S2nTlsProxyServerConfig {
    ...
    fn diff_action(&self, new: &AnyServerConfig) -> ServerConfigDiffAction {
        let AnyServerConfig::S2nTlsProxy(new) = new else {
