...
use crate::config::server::rustls_proxy::RustlsServiceConfig;

pub(crate) struct RustlsService {
    addrs: SelectiveVec<WeightedValue<SocketAddr>>,
    pick_policy: SelectivePickPolicy,
}

impl TryFrom<&Arc<RustlsServiceConfig>> for RustlsService {
    type Error = anyhow::Error;

    fn try_from(value: &Arc<RustlsServiceConfig>) -> Result<Self, Self::Error> {
        RustlsService::build(value)
    }
}

impl RustlsService {
    fn build(config: &Arc<RustlsServiceConfig>) -> anyhow::Result<Self> {
