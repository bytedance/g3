...
use super::{CommonTaskContext, S2nTlsAcceptTask, S2nTlsHost};
use crate::config::server::s2n_tls_proxy::S2nTlsProxyServerConfig;
...
pub(crate) struct S2nTlsProxyServer {
    config: Arc<S2nTlsProxyServerConfig>,
    ...
    tls_accept_config: s2n_tls::config::Config,

    quit_policy: Arc<ServerQuitPolicy>,
    reload_version: usize,
}

impl S2nTlsProxyServer {
    fn new(
        config: Arc<S2nTlsProxyServerConfig>,
        server_stats: Arc<StreamServerStats>,
        listen_stats: Arc<ListenStats>,
        ...
        let tls_accept_config =
            S2nTlsAcceptTask::build_accept_config().context("failed to build tls accept config")?;
    ...
    pub(crate) fn prepare_initial(config: S2nTlsProxyServerConfig) -> anyhow::Result<ArcServer> {
        let config = Arc::new(config);
        let server_stats = Arc::new(StreamServerStats::new(config.name()));
        let listen_stats = Arc::new(ListenStats::new(config.name()));

        let hosts = config.hosts.try_build_arc(S2nTlsHost::try_build)?;

        let server =
            S2nTlsProxyServer::new(config, server_stats, listen_stats, Arc::new(hosts), 1)?;
        Ok(Arc::new(server))
    }

    fn prepare_reload(&self, config: AnyServerConfig) -> anyhow::Result<S2nTlsProxyServer> {
        if let AnyServerConfig::S2nTlsProxy(config) = config {
            let config = Arc::new(config);
    ...
    async fn run_task(&self, stream: TcpStream, cc_info: ClientConnectionInfo) {
        ...
        if self.config.spawn_task_unconstrained {
            tokio::task::unconstrained(S2nTlsAcceptTask::new(ctx).into_running(
                stream,
                &self.hosts,
                self.tls_accept_config.clone(),
            ))
            .await
        } else {
            S2nTlsAcceptTask::new(ctx)
                .into_running(stream, &self.hosts, self.tls_accept_config.clone())
                .await;
        }
    }
}

impl ServerInternal for S2nTlsProxyServer {
    fn _clone_config(&self) -> AnyServerConfig {
        AnyServerConfig::S2nTlsProxy(self.config.as_ref().clone())
    }
...
impl BaseServer for S2nTlsProxyServer {
    #[inline]
...
#[async_trait]
impl AcceptTcpServer for S2nTlsProxyServer {
...
#[async_trait]
impl AcceptQuicServer for S2nTlsProxyServer {
    #[cfg(feature = "quic")]
    async fn run_quic_task(&self, _connection: Connection, _cc_info: ClientConnectionInfo) {}
}

#[async_trait]
impl Server for S2nTlsProxyServer {
