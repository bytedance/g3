...
use crate::config::server::s2n_tls_proxy::S2nTlsHostConfig;

pub(crate) struct S2nTlsHost {
    pub(super) config: Arc<S2nTlsHostConfig>,
    pub(super) tls_config: s2n_tls::config::Config,
...
impl S2nTlsHost {
    pub(super) fn try_build(config: &Arc<S2nTlsHostConfig>) -> anyhow::Result<Self> {
    ...
    pub(super) fn new_for_reload(&self, config: Arc<S2nTlsHostConfig>) -> anyhow::Result<Self> {
        let tls_config = config.build_tls_config()?;
...
impl NamedValue for S2nTlsHost {
    type Name = str;
