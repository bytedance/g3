...
const DEFAULT_EMIT_INTERVAL: Duration = Duration::from_secs(4);
...
pub(crate) enum AnyInputConfig {
    ...
    Internal(internal::InternalInputConfig),
    ...
    ($f:tt, $v:ty, $p:ty) => {
        pub(crate) fn $f(&self, p: $p) -> $v {
            match self {
                ...
                AnyInputConfig::Internal(s) => s.$f(p),
...
fn load_input(
    ...
    match g3_yaml::key::normalize(input_type).as_str() {
        ...
        "internal" => {
            let input = internal::InternalInputConfig::parse(map, position)
                .context("failed to load this Internal input")?;
            Ok(AnyInputConfig::Internal(input))
        }
        "statsd" => {
