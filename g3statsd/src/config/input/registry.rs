...
static INITIAL_INOUT_CONFIG_REGISTRY: Mutex<HashMap<NodeName, Arc<AnyInputConfig>, FixedState>> =
    Mutex::new(HashMap::with_hasher(FixedState::with_seed(0)));

pub(crate) fn clear() {
    let mut ht = INITIAL_INOUT_CONFIG_REGISTRY.lock().unwrap();
    ht.clear();
}
...
pub(super) fn get(name: &NodeName) -> Option<Arc<AnyInputConfig>> {
    let ht = INITIAL_INOUT_CONFIG_REGISTRY.lock().unwrap();
    ht.get(name).cloned()
}
