...
fn reload_old_unlocked(old: AnyInputConfig, new: AnyInputConfig) -> anyhow::Result<()> {
    ...
    match old.diff_action(&new) {
        ...
        InputConfigDiffAction::UpdateInPlace(flags) => {
            debug!("input {name} reload: will update the existed in place");
...
fn spawn_new_unlocked(config: AnyInputConfig) -> anyhow::Result<()> {
    ...
    let input = match config {
        ...
        AnyInputConfig::Internal(config) => {
            super::internal::InternalInput::prepare_initial(config)?
        }
