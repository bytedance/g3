...
impl SimpleBindRequestEncoder {
    ...
    pub(crate) fn encode(
        ...
        let bind_dn = format!("uid={},{}", username.as_original(), base_dn);
        ...
        self.request_buf
            .extend_from_slice(password.as_original().as_bytes());
...
mod tests {
    ...
    fn encode() {
        ...
        let username = Username::from_original("jdoe").unwrap();
        let password = Password::from_original("secret123").unwrap();
        let base_dn = "ou=People,dc=example,dc=com";

        let request = encoder.encode(&username, &password, base_dn);
        assert_eq!(
            request,
            [
