...
pub(crate) fn reload_only_escaper(name: &str) -> anyhow::Result<()> {
    let ht = RUNTIME_SERVER_REGISTRY.lock().unwrap();
...
pub(crate) fn reload_only_user_group(name: &str) -> anyhow::Result<()> {
    let ht = RUNTIME_SERVER_REGISTRY.lock().unwrap();
    let server = match ht.get(name) {
        Some(server) => server,
        None => return Err(anyhow!("no server with name {name} found")),
...
pub(crate) fn reload_only_auditor(name: &str) -> anyhow::Result<()> {
    let ht = RUNTIME_SERVER_REGISTRY.lock().unwrap();
    let server = match ht.get(name) {
        Some(server) => server,
        None => return Err(anyhow!("no server with name {name} found")),
