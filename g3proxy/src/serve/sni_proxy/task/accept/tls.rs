...
use g3_dpi::parser::tls::{ExtensionType, Record, RecordParseError};
...
{
    loop {
        let record = match Record::parse(clt_r_buf) {
            Ok(r) => r,
        ...
        let ch = record.parse_client_hello().map_err(|_| {
            ServerTaskError::InvalidClientProtocol("invalid tls client hello request")
        })?;
        return match ch.get_ext(ExtensionType::ServerName) {
            Ok(Some(data)) => {
