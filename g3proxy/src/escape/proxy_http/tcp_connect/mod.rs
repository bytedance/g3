...
impl ProxyHttpEscaper {
    ...
    async fn tcp_connect_to(
        ...
        match peer_proxy.host() {
            Host::Ip(ip) => {
                // If sticky is requested and proxy_nodes are static IPs, do HRW over the list and persist mapping
                if let Some(decision) = task_notes.sticky()
                    && decision.enabled()
                    && !decision.rotate
                {
                    if self.static_proxy_ip_ports.len() >= 2 {
                        let ips: Vec<IpAddr> = self.static_proxy_ip_ports.iter().map(|(ip, _)| *ip).collect();
                        if let Some((pick, key, _hit)) = crate::sticky::choose_sticky_ip(decision, task_conf.upstream, &ips).await {
                            let port = self
                            ...
                            match self.fixed_try_connect(peer, task_conf, tcp_notes, task_notes).await {
                                Ok(stream) => {
                    ...
                    if let Some((pick, key, cache_hit)) = crate::sticky::choose_sticky_ip(decision, task_conf.upstream, &ips).await {
                        ...
                        match self.fixed_try_connect(peer, task_conf, tcp_notes, task_notes).await {
                            Ok(stream) => {
                                ...
                                crate::sticky::redis_set_ip(&key, pick, ttl).await;
                                let _ = cache_hit; // reserved for future stats
                                return Ok(stream);
                            }
                            Err(_) => {
