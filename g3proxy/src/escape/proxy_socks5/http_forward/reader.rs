...
#[pin_project]
pub(super) struct ProxySocks5HttpForwardReader<R: AsyncRead> {
    #[pin]
    inner: LimitedBufReader<R>,
}

impl<R> ProxySocks5HttpForwardReader<R>
where
    R: AsyncRead + Unpin,
{
    pub(super) fn new(ups_r: LimitedBufReader<R>) -> Self {
        ProxySocks5HttpForwardReader { inner: ups_r }
    }
...
impl<R> AsyncRead for ProxySocks5HttpForwardReader<R>
where
    R: AsyncRead,
{
    fn poll_read(
...
impl<R> AsyncBufRead for ProxySocks5HttpForwardReader<R>
where
    R: AsyncRead,
{
...
#[async_trait]
impl<R> HttpForwardRead for ProxySocks5HttpForwardReader<R>
where
