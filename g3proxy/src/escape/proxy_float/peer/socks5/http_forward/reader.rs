...
#[pin_project]
pub(super) struct Socks5PeerHttpForwardReader<R: AsyncRead> {
    #[pin]
    inner: LimitedBufReader<R>,
}

impl<R> Socks5PeerHttpForwardReader<R>
where
    R: AsyncRead + Unpin,
{
    pub(super) fn new(ups_r: LimitedBufReader<R>) -> Self {
        Socks5PeerHttpForwardReader { inner: ups_r }
    }
...
impl<R> AsyncRead for Socks5PeerHttpForwardReader<R>
where
    R: AsyncRead,
{
    fn poll_read(
...
impl<R> AsyncBufRead for Socks5PeerHttpForwardReader<R>
where
    R: AsyncRead,
{
...
#[async_trait]
impl<R> HttpForwardRead for Socks5PeerHttpForwardReader<R>
where
