...
#[pin_project]
pub(super) struct DirectFloatHttpForwardReader<R: AsyncRead> {
    #[pin]
    inner: LimitedBufReader<R>,
}

impl<R> DirectFloatHttpForwardReader<R>
where
    R: AsyncRead + Unpin,
{
    pub(super) fn new(ups_r: LimitedBufReader<R>) -> Self {
        DirectFloatHttpForwardReader { inner: ups_r }
    }
...
impl<R> AsyncRead for DirectFloatHttpForwardReader<R>
where
    R: AsyncRead,
{
    fn poll_read(
...
impl<R> AsyncBufRead for DirectFloatHttpForwardReader<R>
where
    R: AsyncRead,
{
...
#[async_trait]
impl<R> HttpForwardRead for DirectFloatHttpForwardReader<R>
where
