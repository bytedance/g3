...
mod tests {
    ...
    #[test]
    fn as_u32_err() {
        let v = ValueRef::String(Utf8StringRef::from("\0ff\0fe"));
    ...
    fn as_weighted_name_string_ok() {
        ...
        let v = vec![
            ...
            (
                ValueRef::String(Utf8StringRef::from("other")),
                ValueRef::String(Utf8StringRef::from("value")),
            ),
        ];
        let v = ValueRef::Map(v);
        let pv = as_weighted_name_string(&v).unwrap();
    ...
    #[test]
    fn as_weighted_name_string_err() {
        let v = vec![(
            ValueRef::String(Utf8StringRef::from("weight")),
            ValueRef::Integer(Integer::from(1)),
        )];
        let v = ValueRef::Map(v);
        assert!(as_weighted_name_string(&v).is_err());
    }
}
