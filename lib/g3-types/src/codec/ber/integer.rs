...
impl BerInteger {
    pub fn parse<const IDENTIFIER: u8>(data: &[u8]) -> Result<Self, BerIntegerParseError> {
        if !data.is_empty() {
        ...
        if data.len() < 3 {
            return Err(BerIntegerParseError::NeedMoreData(3 - data.len()));
        }
        ...
        match length.value() {
            1 => {
                ...
                Ok(BerInteger {
                    value: i8::from_be_bytes([left[0]]) as i64,
            ...
            2 => {
                ...
                Ok(BerInteger {
                    value: i16::from_be_bytes([left[0], left[1]]) as i64,
            ...
            3 => {
                ...
                let value = if left[0] & 0x80 != 0 {
                    i32::from_be_bytes([0x80, 0, left[0] & 0x7F, left[1]]) as i64
                } else {
                    i64::from(u32::from_be_bytes([0, left[0], left[1], left[2]]))
                };
                Ok(BerInteger {
                    value,
            ...
            4 => {
                ...
                let value = i32::from_be_bytes([left[0], left[1], left[2], left[3]]) as i64;
                Ok(BerInteger {
                    value,
            ...
            5 => {
                ...
                let value = if left[0] & 0x80 != 0 {
                    ...
                    i64::from_be_bytes([0, 0, 0, left[0], left[1], left[2], left[3], left[4]])
                };
                Ok(BerInteger {
                    value,
            ...
            6 => {
                ...
                let value = if left[0] & 0x80 != 0 {
                    ...
                    i64::from_be_bytes([0, 0, left[0], left[1], left[2], left[3], left[4], left[5]])
                };
                Ok(BerInteger {
                    value,
            ...
            7 => {
                ...
                let value = if left[0] & 0x80 != 0 {
                    ...
                    i64::from_be_bytes([
                        0, left[0], left[1], left[2], left[3], left[4], left[5], left[6],
                    ])
                };
                Ok(BerInteger {
                    value,
            ...
            _ => Err(BerIntegerParseError::InvalidValueBytes),
        }
    }
...
mod tests {
    ...
    #[test]
    fn parse() {
        let e = BerInteger::parse::<2>(&[0x02]).unwrap_err();
        assert_eq!(e, BerIntegerParseError::NeedMoreData(2));

        let e = BerInteger::parse::<2>(&[0x03, 0x01, 0x02]).unwrap_err();
        assert_eq!(e, BerIntegerParseError::InvalidType);
        let e = BerInteger::parse::<2>(&[0x02, 0x00, 0x02]).unwrap_err();
        assert_eq!(e, BerIntegerParseError::InvalidValueBytes);

        let v = BerInteger::parse::<2>(&[0x02, 0x01, 0x02]).unwrap();
        assert_eq!(v.value, 2);
        assert_eq!(v.encoded_len(), 3);
        let v = BerInteger::parse::<2>(&[0x02, 0x81, 0x01, 0x02]).unwrap();
        assert_eq!(v.value, 2);
        assert_eq!(v.encoded_len(), 4);
        let e = BerInteger::parse::<2>(&[0x02, 0x01, 0x82]).unwrap();
        assert_eq!(v.value, -2);
        assert_eq!(v.encoded_len(), 3);

        let v = BerInteger::parse::<2>(&[0x02, 0x02, 0x01, 0x02]).unwrap();
        assert_eq!(v.value, 0x0102);
        assert_eq!(v.encoded_len(), 4);
        let e = BerInteger::parse::<2>(&[0x02, 0x02, 0x01]).unwrap_err();
        assert_eq!(e, BerIntegerParseError::NeedMoreData(1));
        let e = BerInteger::parse::<2>(&[0x02, 0x02, 0x81, 0x02]).unwrap();
        assert_eq!(v.value, -0x0102);
        assert_eq!(v.encoded_len(), 4);

        let v = BerInteger::parse::<2>(&[0x02, 0x03, 0x01, 0x01, 0x02]).unwrap();
        assert_eq!(v.value, 0x010102);
        assert_eq!(v.encoded_len(), 5);
        let e = BerInteger::parse::<2>(&[0x02, 0x03, 0x01]).unwrap_err();
        assert_eq!(e, BerIntegerParseError::NeedMoreData(2));
        let e = BerInteger::parse::<2>(&[0x02, 0x03, 0x81, 0x01, 0x02]).unwrap();
        assert_eq!(v.value, -0x010102);
        assert_eq!(v.encoded_len(), 5);

        let v = BerInteger::parse::<2>(&[0x02, 0x04, 0x01, 0x01, 0x01, 0x02]).unwrap();
        assert_eq!(v.value, 0x01010102);
        assert_eq!(v.encoded_len(), 6);
        let e = BerInteger::parse::<2>(&[0x02, 0x04, 0x01, 0x01]).unwrap_err();
        assert_eq!(e, BerIntegerParseError::NeedMoreData(2));
        let e = BerInteger::parse::<2>(&[0x02, 0x04, 0x81, 0x01, 0x01, 0x02]).unwrap();
        assert_eq!(v.value, -0x01010102);
        assert_eq!(v.encoded_len(), 6);
    }
}
