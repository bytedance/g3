...
HTTPS_PROXY2="https://g3proxy.local:9443"
...
python3 "${PROJECT_DIR}/g3proxy/ci/python3+curl/test_httpbin.py" -x ${HTTPS_PROXY2} -T http://httpbin.local --proxy-ca-cert ${SSL_CERT_FILE}
...
python3 "${PROJECT_DIR}/g3proxy/ci/python3+requests/test_httpbin.py" -x ${HTTPS_PROXY2} -T http://httpbin.local
